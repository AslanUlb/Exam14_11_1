...
Ticket::Ticket()
{
	...
	if (is) {
		while (!is.eof()) {
			is >> key;
			if (!key.empty()) {
				temp_ticket.numFlight = key;
				is >> temp_ticket.pointSrc;
				is >> temp_ticket.pointDst;

				is >> temp_ticket.dateFlight.day;
				is >> temp_ticket.dateFlight.month;
				is >> temp_ticket.dateFlight.year;

				is >> temp_ticket.timeUp.hour;
				is >> temp_ticket.timeUp.min;
				is >> temp_ticket.timeUp.sec;

				is >> temp_ticket.timeDown.hour;
				is >> temp_ticket.timeDown.min;
				is >> temp_ticket.timeDown.sec;

				is >> temp_ticket.numberTicket;
